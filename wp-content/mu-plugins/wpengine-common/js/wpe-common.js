...
jQuery(document).ready(function($) {    	    			
	...
	if(filename == 'update-core.php' && $('form.upgrade').length > 0 && wpe.popup_disabled != 1 ) {
		...
		// Custom inline backup message for 4.2 shiny updates
  		if($('.update-message').length > 0 ) {
	  		$('.update-message').each(function(i,obj) {

			var $message = '<div class="error inline">Before taking this action, we at WP Engine recommend that you <a target="_blank" href="https://my.wpengine.com/installs/'+wpe.account+'/backup_points">create a Restore Point of your site</a> in the User Portal. This will allow you undo this action within minutes.</div>';
		 	$(this).append($message);
	  		});	
	 	 }
	...
	$('#doaction').attr('onclick','wpe_validate_bulk_form();');
		...
		$('a.install-now').click(function(e) {
			...
			var installConfirm = confirm(plugininstallL10n.ays);
			if(installConfirm) {
				wpe_upgrade_link($(this).attr('href'));
			};
			// Stop WordPress core's default confirm popup
			e.preventDefault();
